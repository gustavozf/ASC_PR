...
escala1 = 'zl'
dir_origem_tipo1 = '/home/gustavozf/Documentos/Projeto/Saidas/OUT/zl_lbp_1z_1s/Mono/'
# Diretório e tipo de origem
tipo2 = 'rlbp'
dir_origem_tipo2 = '/home/gustavozf/Documentos/Projeto/Saidas/OUT/zl_rlbp_1z_1s/Spectrograms2/'

# Diretório dos arquivos gerados com as regras de fusão
dir_fusao_predict = '/home/gustavozf/Documentos/Projeto/Saidas/OUT/fusao_lbp_1z_1s_rlbp/'

# Define as regras usadas
regras = ['maximo', 'produto', 'soma']
...
for f in range(1, qtde_folds+1):
	...
	#fold_original = dir_origem_tipo1 + 'fold-' + ("%02d" % f) + escala1 + '-1-' + tipo1 + '.svm'
	#fold_results[0] = dir_origem_tipo1 + 'fold-' + ("%02d" % f) + escala1 + '-fusao-soma-' + regras[0] + '.svm.predict'
	...
	fold_original = dir_original + 'fold' + str(f) + '-' + escala1 + '-0-0-' + tipo1 + '.svm'
	fold_results[0] = dir_origem_tipo + 'fold-' + str(f) + '-' +  escala + '-fusao-' + regras[0] + '-' + tipo1 + '-' + tipo2 +  '.svm.predict'
	fold_results[1] = dir_origem_tipo + 'fold-' + str(f) + '-' +  escala + '-fusao-' + regras[1] + '-'  + tipo1 + '-' + tipo2 + '.svm.predict'
	fold_results[2] = dir_origem_tipo + 'fold-' + str(f) + '-' +  escala + '-fusao-' + regras[2] + '-'  + tipo1 + '-' + tipo2 + '.svm.predict'
