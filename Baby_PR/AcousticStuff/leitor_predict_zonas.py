...
# Se for Linux...
if not is_win:
	dir_origem_tipo = '/home/gustavozf/Documentos/Projeto/Saidas/OUT/mel_rlbp_15z_1s/'
	dir_original = '/home/gustavozf/Documentos/Projeto/Saidas/OUT/'
# Se for Windows...
else:
    dir_origem_tipo = 'D:/Mestrado/'

#####################################################################
# Nome da pasta com o conteúdo a ser analisado
dir_origem_tipo += 'Mono/predicts/'
# Quantidade de folds
qtde_folds = 4
# Diretório e tipo de origem
tipo = 'rlbp'
# Define o nome da escala usada caso exista no nome do arquivo
# Ex.: 'mel', 'zl', '' (vazio)
escala = 'mel'
#####################################################################
...
for f in range(1, qtde_folds+1):
	...
	# Só abre o primeiro porque as Z zonas tem a mesma estrutura de amostras e classes
	fold_original = dir_original + 'fold' + str(f) + '-' + 'zl' + '-0-0-rlbp.svm'
	fold_results[0] = dir_origem_tipo + 'fold-' + str(f) + '-' +  escala + '-fusao-' + regras[0] + '-' + tipo + '.svm.predict'
	fold_results[1] = dir_origem_tipo + 'fold-' + str(f) + '-' +  escala + '-fusao-' + regras[1] + '-' + tipo + '.svm.predict'
	fold_results[2] = dir_origem_tipo + 'fold-' + str(f) + '-' +  escala + '-fusao-' + regras[2] + '-' + tipo + '.svm.predict'
