...
function feat = process_audiofile(input_filename, opts)
	...
	if (strcmp(input_extension,'au')) 
		filesize = auread(input_filename,'size');
		[dummy,sample_freq,n_bits] = auread(input_filename,1);
	else	% assume wav
		filesize = wavread(input_filename,'size');
		[dummy,sample_freq,n_bits] = wavread(input_filename,1);
	end		
