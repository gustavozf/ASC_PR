...
def converterSVM(dir_origem, folds_qtde, tipo, zona, escala, early_fusion, early, segmento):
        ...
        for i in range(0, total_indices): # porque o range pára 1 antes do final
            for f in range(1, folds_qtde+1):
                ...
                # Abre o arquivo LBP
                txt = dir_origem + "fold" + str(f) + '/' + indices[i] + '/' + escala + '-' + indices[i] + zona + segmento  + early +'.txt'
                arq = open(txt, 'r')
...
if not is_win:
        ...
        dir_origem = '/home/gustavozf/Documentos/UEM/Projetos/Baby/base_americana/04_Features/audios_04/'
# Se for Linux...
else:
    dir_origem = 'E:\\'
...
if early_fusion:
    ...
    early = ''
### Origem da base ###
dir_origem += str(folds_qtde) + '_folds/'+ frequencia+'Hz_' + amplitude+"dB/"
dir_origem += tipo + '_' + escala + '_' + zonas + 'z_' + segmentos + "s/"
####################################################################y#
#pastas = ['Mono/', 'Spectrograms1/', 'Spectrograms2/']
pastas = ['Left_Channel/', 'Right_Channel/']

tt = ['test/', 'train/']

for i in range(0, len(pastas)): #numero de pastas
        for j in range(0, 2):
